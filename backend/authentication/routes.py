...
urlpatterns = [
    ...
    path("reset_password/email/check/", ResetPasswordEmailCheck.as_view({"post": "check"}), name="reset_password_email_check"),
    path("reset_password/link/check/<str:token>/<str:uidb64>/", ResetPassword.as_view({"get": "check"}), name="reset_password_link_check"),
    path("reset_password/link/change/<str:token>/", ResetPassword.as_view({"post": "change"}), name="reset_password_change"),
]
